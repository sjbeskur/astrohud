...
| `C-115` | P1 | S | Next version | Add owner recovery that survives cleared browser cookies. During onboarding, issue a private recovery link that can be saved in a password manager or secure notes; allow an authenticated owner to rotate it and invalidate the previous link. Evaluate passkeys or verified email before a broader release. | `C-102`, `C-104` |
